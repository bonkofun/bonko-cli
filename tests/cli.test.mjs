...
import { buildRuntime } from '../engine/runtime-build.mjs';
const executable = path.join(toolRoot, 'bin/bonko.mjs');

test('version boundaries and standard command aliases', () => {
  for (const value of ['18.20.0', '20.19.0', '22.11.9', 'v22.12.0-rc.1', 'invalid']) assert.equal(supportsNode(value), false);
  for (const value of ['22.12.0', 'v22.23.2', '24.0.0', '26.0.0']) assert.equal(supportsNode(value), true);
  assert.equal(parseArgs([]).command, 'help');
  assert.equal(parseArgs(['-v']).command, 'version');
  assert.deepEqual(parseArgs(['dev', '--help']).operands, ['dev']);
  for (const args of [['toString'], ['wat'], ['new'], ['dev','--port'], ['dev','--port','0'], ['dev','--port','65536'], ['pack','--no-open'], ['build','surprise'], ['dev','--wat']]) assert.throws(() => parseArgs(args));
});

test('help/version work outside a project, errors return nonzero and valid JSON', () => {
  for (const args of [['help'], ['--help'], ['version'], ['-v']]) assert.equal(spawnSync(process.execPath,[executable,...args], { cwd: tmpdir(), encoding:'utf8' }).status,0);
  const result = spawnSync(process.execPath,[executable,'unknown','--json'], {encoding:'utf8'});
  assert.equal(result.status,1); assert.equal(JSON.parse(result.stderr).ok,false);
});
...
test('new creates flat projects exclusively and resolves them from nested folders', async () => {
  ...
  try {
    ...
    await writeFile(path.join(project.root,'bonko.json'),JSON.stringify({...config,cliVersion:'99.0.0'}));
    await assert.rejects(findProject(project.root),/pinned/);
...
test('tool-owned dependencies build React/Motion without project node_modules and reject unsafe sources', async () => {
  ...
  try {
    ...
    await assert.rejects(buildRuntime(project.root,project.slug),error=>error.code==='TYPESCRIPT_FAILED');
    await writeFile(sourcePath,original); await symlink(path.join(parent,'missing'),path.join(project.root,'assets/linked.png'));
