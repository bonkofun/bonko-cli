...
async function run(file,args,cwd) {
  return new Promise((resolve,reject)=>{
    const child=spawn(file,args,{cwd,stdio:['ignore','pipe','pipe']});let stdout='',stderr='';
    child.stdout.on('data',data=>stdout+=data);child.stderr.on('data',data=>stderr+=data);
    child.once('error',reject);child.once('exit',code=>code===0?resolve(stdout):reject(new Error(`${file} exited ${code}: ${stderr}\n${stdout}`)));
  });
}

test('prebuilt Studio edits, reloads source, rejects unauthorized API calls and recovers from errors', {timeout:60000}, async()=>{
  const parent=await mkdtemp(path.join(tmpdir(),'bonko-cli-preview-'));let server,browser;
  try {
    ...
    assert.equal((await fetch(server.origin+'/',{headers:{Origin:'https://example.test'}})).status,403);
    ...
    const frame=page.frameLocator('iframe');await expect(frame.getByRole('heading')).toHaveText('Alex');
    ...
    for(const width of [375,390,430]) {
      ...
      await expect.poll(()=>page.frames().find(f=>f!==page.mainFrame()).evaluate(()=>innerWidth)).toBe(width);
      await page.setViewportSize({width,height:1000});
      assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    }
    const source=path.join(project.root,'src/main.tsx'), original=await readFile(source,'utf8');
  ...
  } finally {await browser?.close();await server?.close();await rm(parent,{recursive:true,force:true});}
});

test('real release installs without source repos, then new/build/dev/check/pack work', {timeout:240000}, async()=>{
  ...
  try {
    ...
    assert.equal(result.verified,false);await access(path.join(result.directory,'runtime/entry.js'));
    await assert.rejects(access(path.join(project,'dist')),{code:'ENOENT'});
    const report=JSON.parse(await run(executable,['check','--no-download','--json'],project));
    assert.equal(report.ok,true);assert.ok(report.checks.includes('authored-static-reduced-motion'));
    const packed=JSON.parse(await run(executable,['pack','--no-download','--json'],project));
    assert.equal(packed.ok,true);assert.ok((await readFile(packed.files[0])).length>1000);
