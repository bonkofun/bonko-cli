...
- **Cover and opening poster**: Decode the final catalog cover and verify a 3:2 landscape ratio, correct spelling, clean edges and legibility in desktop and mobile two-column thumbnails. Recommend 1200 × 800 pixels, but accept other sharp 3:2 resolutions such as 900 × 600, 1536 × 1024 or 1800 × 1200; do not reject a cover solely for differing from the recommended pixel dimensions. Check safe margins under rounded corners and hover zoom, plus encoded byte size and package budgets. Confirm the catalog artwork represents the runtime. For Receiver openings, inspect the separately declared `receiverOpeningPoster`: same still opening, visible tap hint, no private content or captured preview/host controls. Verify `ready` is still and silent and one host gesture reveals without a second click.
