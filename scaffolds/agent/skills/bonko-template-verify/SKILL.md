...
- **Cover and opening poster**: Decode the final catalog cover and verify a 4:5 portrait ratio, correct spelling, clean edges and legibility in compact desktop four-column and mobile two-column thumbnails. Recommend 1200 × 1500 pixels, but accept other sharp 4:5 resolutions such as 800 × 1000, 1024 × 1280 or 1600 × 2000; do not reject a cover solely for differing from the recommended pixel dimensions. Check approximately 8% safe margins, complete short titles at thumbnail size, rounded-corner clearance, encoded byte size and package budgets. Host-owned price badges, buttons and coral/lime card frames must not be baked into the artwork. Legacy published landscape covers remain compatible and should display uncropped in the host; do not modify immutable releases to satisfy the new authoring recommendation. Confirm the catalog artwork represents the runtime. For Receiver openings, inspect the separately declared `receiverOpeningPoster`: same still opening, visible tap hint, no private content or captured preview/host controls. Verify `ready` is still and silent and one host gesture reveals without a second click.
