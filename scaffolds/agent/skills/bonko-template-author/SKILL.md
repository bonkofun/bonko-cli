...
- Deliver a dedicated **4:5 portrait catalog cover**, preferably WebP. **1200 × 1500 px is the recommended production size, not an exact-resolution requirement**; 800 × 1000, 1024 × 1280 and 1600 × 2000 px are also suitable when sharp at the intended display size and within package budgets. New main-site catalog and draw-deck artwork uses a 4:5 portrait composition. Compose dedicated artwork for this ratio rather than stretching a phone screenshot or cropping a landscape cover. The catalog preserves legacy landscape artwork at its natural ratio without cropping; new covers should use the portrait composition rather than adding letterbox bars. Use the same subject, materials and atmosphere as the actual template; a cover must not promise a different experience.
- For generated covers, specify the exact short display text, legible typography, safe margins, subject placement and exclusions in the prompt. Inspect spelling and thumbnail legibility after generation, including compact desktop four-column and mobile two-column cards. Keep important text and subjects inside an approximately 8% safe margin for rounded corners and small-thumbnail legibility; omit long descriptions. The host supplies the cream surface, coral/lime border, price badge, title and preview control: do not bake them into the image. Match the actual template art direction rather than forcing coral/lime into every template. High-resolution source artwork may be downsampled; verify the final file's decoded dimensions, sharpness and byte size rather than assuming the generator honored its prompt. Use embossed lettering when appropriate to the art direction; never bake recipient content into catalog artwork.
...
## Playback duration and photo pacing

- Finish the automatic sequence within **15 seconds of active playback**, measured from the gesture that starts playback through natural completion. Include opening, transitions, photo holds and the final audio hold; exclude asset loading, waiting for a gesture and paused time. This is an authoring requirement, stricter than the host's existing 30-second safety deadline; do not modify or bypass SDK safeguards.
- Keep each photo fully visible and readable for approximately **3 seconds**. Budget entrance and exit separately; a mostly obscured transition does not count as its reading time. Shorter templates need not stretch to 15 seconds.
- Prefer at most **4 photos for a one-at-a-time sequence**, leaving room for opening and transitions. For **5–10 photos**, use readable simultaneous groups or collages, holding each group for approximately 3 seconds. Show every supplied photo and its caption; do not silently drop photos, shrink them into unreadable thumbnails or reduce `maxPhotos` without changing the template's actual supported capacity.
- Example budget for four sequential photos: 1-second opening + four 3-second photo holds + at most 2 seconds total transitions and closing = at most 15 seconds. Plan overlapping visual/audio timelines rather than adding their durations together.
- Keep each delivered audio asset at **10.0 seconds or less**, including its tail. Schedule cues and validated startup allowance so all sound resolves within the 15-second sequence. Choose or author a shorter track when needed; do not cut a playing track off or loop it to fill the animation budget.
- After completion, preserve a still final presentation with the photos, captions, recipient, message and optional sender available to read. Use a contained readable layout when needed; 15 seconds limits automatic playback, not the user's reading time. Skip and reduced motion show the complete static result immediately.
...
| :-------------------------------- | :--------------------------------------------------------- | :----------- | :---------- | :------------------------------------------------------------------------------------ |
| **Catalog Cover (`cover`)**       | 1200 × 1500 px recommended (4:5; other 4:5 sizes accepted) | WebP         | 50–120 KB   | Catalog store listing; high visual richness, 3D embossed lettering, celebratory mood. |
