...
const allowed = new Set(["react", "react/jsx-runtime", "react-dom/client", "motion/react", "@bonko/template-sdk/runtime-client"]);
...
async function tree(root, prefix = "", budget = { bytes: 0, count: 0 }) {
  ...
  for (const name of await readdir(folder)) {
    if (!prefix && !["manifest.json", "LICENSE.md", "test.json", "src", "assets"].includes(name)) continue;
    ...
    if (stat.isDirectory()) Object.assign(result, await tree(root, relative, budget));
      ...
      if (++budget.count > LIMITS.files || budget.bytes > LIMITS.expanded) throw new Error("Source budget exceeded");
      result[relative] = await readFile(file);
    } else throw new Error("Unsupported source file");
  }
  return result;
}
export function checkSourceImports(files) {
  for (const [name, bytes] of Object.entries(files)) {
    if (name.endsWith(".css")) {
      if (/@import|url\s*\(/i.test(bytes.toString("utf8"))) throw new Error("CSS resources must use runtime asset IDs, not imports or URLs");
      continue;
    }
    if (!/\.(ts|tsx)$/.test(name)) throw new Error("Only TypeScript/TSX/CSS source files are allowed");
    ...
    const visit = node => {
      if (ts.isImportEqualsDeclaration(node)) throw new Error("Import assignments are not supported");
      if (ts.isMetaProperty(node)) throw new Error("Environment and import.meta access is not allowed in templates");
      if (ts.isCallExpression(node) && (node.expression.kind === ts.SyntaxKind.ImportKeyword || (ts.isIdentifier(node.expression) && ["require", "eval"].includes(node.expression.text)))) throw new Error("Dynamic imports, require and eval are not supported");
      if ((ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) && node.moduleSpecifier) {
        ...
        if (target.startsWith(".")) {
          ...
          if (resolved.startsWith("../") || resolved === ".." || target.includes("\\") || target.includes("?")) throw new Error("Imports must stay inside src");
        } else if (!allowed.has(target)) throw new Error(`Dependency requires platform review: ${target}`);
      }
      if (ts.isImportTypeNode(node)) throw new Error("Use approved static type imports, not import() types");
...
export function typecheckRuntime(folder, source) {
  ...
  const snapshots = new Map(Object.entries(source).map(([name, bytes]) => [path.resolve(root, name), bytes.toString("utf8")]));
  const host = ts.createCompilerHost(options);
  host.resolveModuleNames = (names, containingFile) => names.map(name => {
    const from = containingFile.startsWith(root + path.sep) && allowed.has(name)
      ? path.join(toolRoot, "__template_types__.tsx") : containingFile;
  ...
  const program = ts.createProgram([...snapshots.keys()].filter(file => /\.tsx?$/.test(file)), options, host);
  ...
  if (diagnostics.length) throw new RuntimeBuildError("TYPESCRIPT_FAILED", "Template TypeScript checks failed", diagnostics);
}

/** Local author code is built here, never by an Admin upload handler. */
export async function buildRuntime(root, slug) {
  if (!/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/.test(slug)) throw new Error("Invalid slug");
  const folder = root;
  const all = await tree(folder);
  if (!all["manifest.json"] || !all["LICENSE.md"] || !all["src/main.tsx"]) throw new Error("Expected manifest.json, LICENSE.md and src/main.tsx");
  ...
  const source = Object.fromEntries(Object.entries(all).filter(([name]) => name.startsWith("src/")).map(([name, bytes]) => [name.slice(4), bytes]));
  checkSourceImports(source);
  typecheckRuntime(folder, source);
  const result = await build({
    root: folder, configFile: false, envDir: false, logLevel: "silent",
    resolve: { alias: [...allowed].map(name => ({ find: name, replacement: toolRequire.resolve(name) })).sort((a, b) => b.find.length - a.find.length) },
    ...
    build: { write: false, sourcemap: false, minify: true,
      lib: { entry: path.join(folder, "src/main.tsx"), name: "BonkoTemplate", formats: ["iife"], cssFileName: "style" } },
  });
  ...
  if (chunks.length !== 1 || chunks[0].imports.length || chunks[0].dynamicImports.length) throw new Error("Runtime must be a single self-contained script");
  const css = outputs.filter(file => file.type === "asset");
  if (css.length > 1 || css.some(file => !file.fileName.endsWith(".css"))) throw new Error("Use logical asset IDs, not imported media");
  const dependencies = {};
  ...
  const files = {
    "manifest.json": Buffer.from(JSON.stringify({ ...manifest, ...(css.length ? { stylesheet: "runtime/style.css" } : {}) })),
    "LICENSE.md": all["LICENSE.md"], "runtime/entry.js": Buffer.from(chunks[0].code),
