...
The **Config** tab reads and saves `manifest.json`: template name, description, author name, tags, photo count (1–10), a USD price from $0 to $9.90. Saving preserves other manifest fields and uses revision checks to reject stale edits. **Reload from file** discards unsaved form changes and loads external edits. `tsconfig.json` remains compiler configuration and is not modified.

Price is stored as `config.suggestedPriceCents` (integer cents) and `config.suggestedPriceCurrency` (`USD`). The server derives `access`: zero is `free`, and a positive price is `premium`. There is no separate Access control. Pricing remains authoring metadata for platform review, not an active checkout, and is omitted from runtime initialization. The upload limit is independent of the authored demo count. Changing it preserves all `previewPhotoN` / `previewCaptionN` entries and assets. Preview uses up to the new limit from the existing demo set; increasing the limit does not require generating more demo photos. Each declared demo must still reference a valid image and have a matching caption.
