...
The CLI validates declared preview-photo references and requires a contiguous, distinct demo sequence with a caption for every declared demo when any previewPhoto field is present. This sequence is validated independently of the user upload limit, including retained demos above that limit. Drafts without these fields remain buildable, but finished authoring deliverables require generated demonstration photos. Normal asset bundling includes the declared images in `.bonko.zip`; merely placing files in assets/ does not include them. Inspect the delivered ZIP, not only the source directory.
