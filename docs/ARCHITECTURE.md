...
The Photo panel accepts a batch of local PNG, JPEG or WebP files (10 MiB per file), bounded by the template's `config.maxPhotos` (default 1, maximum 10). This upload limit is independent of the retained authored demo set; preview uses at most the limit, while all declared demo references and captions remain validated and preserved. Select a photo to preview and crop it, drag the dedicated handle to reorder, or remove individual photos. Focused handles also support Up/Down and Home/End; descriptions and thumbnails remain ordinary input and selection targets. Each photo retains its own framing. Replay supplies the ordered image bytes and `bonkoPhotoCount`/`bonkoPhotoTransformN` configuration to compatible templates. Photos stay in browser memory and are excluded from template packages.
